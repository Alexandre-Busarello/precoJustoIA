...
/**
 * Script para executar atualização BÁSICA de BDRs
 * Apenas dados financeiros atuais (sem históricos)
 * 
 * Usage:
...
console.log('🌎 Executando atualização BÁSICA de BDRs...');
console.log(`📍 Ambiente: ${isProduction ? 'PRODUÇÃO' : 'LOCAL'}`);
console.log(`🔗 URL: ${baseUrl}/api/cron/update-portfolio-assets?mode=bdr`);
console.log('ℹ️  Modo básico: apenas dados financeiros atuais');
console.log('⏱️  Tempo estimado: 3-5 minutos para 80+ BDRs');
console.log('');
...
console.log('⏳ Executando atualização básica... (isso pode levar 3-5 minutos)');
console.log('');

const startTime = Date.now();

const req = client.request(`${baseUrl}/api/cron/update-portfolio-assets?mode=bdr`, options, (res) => {
  let data = '';
  ...
  res.on('end', () => {
    ...
    if (res.statusCode === 200) {
      try {
        ...
        if (result.summary) {
          console.log('📈 Estatísticas BDR Básico:');
        ...
        console.log('');
        console.log('ℹ️  Dados básicos incluem:');
        console.log('  • Indicadores de valuation atuais');
        console.log('  • Dados financeiros TTM');
        console.log('  • Informações da empresa');
        console.log('  • Cotação atual');
