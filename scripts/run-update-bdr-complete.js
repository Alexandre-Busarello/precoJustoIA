...
/**
 * Script para executar atualização COMPLETA de BDRs
 * Inclui dados históricos, preços e dividendos
 * 
 * Usage:
...
console.log('🌎 Executando atualização COMPLETA de BDRs...');
console.log(`📍 Ambiente: ${isProduction ? 'PRODUÇÃO' : 'LOCAL'}`);
console.log(`🔗 URL: ${baseUrl}/api/cron/update-portfolio-assets?mode=bdr-complete`);
console.log('⚠️  ATENÇÃO: Modo completo inclui dados históricos, preços e dividendos');
console.log('⏱️  Tempo estimado: 10-15 minutos para 80+ BDRs');
console.log('');
...
console.log('⏳ Executando atualização completa... (isso pode levar 10-15 minutos)');
console.log('');

const startTime = Date.now();

const req = client.request(`${baseUrl}/api/cron/update-portfolio-assets?mode=bdr-complete`, options, (res) => {
  let data = '';
  ...
  res.on('end', () => {
    ...
    if (res.statusCode === 200) {
      try {
        ...
        if (result.summary) {
          console.log('📈 Estatísticas BDR Completo:');
          console.log(`  • Total de BDRs: ${result.summary.totalTickers}`);
          console.log(`  • Processados: ${result.summary.processedTickers}`);
          console.log(`  • Falharam: ${result.summary.failedTickers?.length || 0}`);
          console.log(`  • Dados históricos: ${result.summary.updatedHistoricalPrices}`);
          console.log(`  • Dividendos históricos: ${result.summary.updatedDividends}`);
        ...
        console.log('');
        console.log('🎉 Dados completos incluem:');
        console.log('  • Balanços patrimoniais históricos');
        console.log('  • Demonstrações de resultado históricos');
        console.log('  • Fluxos de caixa históricos');
        console.log('  • Preços históricos (últimos 2 anos)');
        console.log('  • Dividendos históricos (últimos 5 anos)');
