...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (isMonthlyReport) {
    return {
      ...
      description: `Análise mensal completa com Inteligência Artificial do BDR ${report.company.name}.`,
    };
  }
  ...
  return {
    ...
    description: isPremium 
      ? `Análise detalhada das mudanças fundamentais no BDR ${report.company.name}. Score: ${report.previousScore} → ${report.currentScore}`
      : `Análise detalhada das mudanças fundamentais no BDR ${report.company.name}. Upgrade para Premium para ver scores completos.`,
  };
}
...
export default async function ReportDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="container max-w-5xl mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-6">
        <Link href={`/bdr/${ticker.toLowerCase()}`}>
