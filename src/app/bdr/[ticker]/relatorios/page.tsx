...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `Histórico completo de relatórios e análises do BDR ${company.name}. Acompanhe a evolução do ativo ao longo do tempo.`,
  };
}
        ...
        <div className="space-y-4">
          {reports.map((report) => {
            ...
            return (
              <Link 
                key={report.id} 
                href={`/bdr/${ticker.toLowerCase()}/relatorios/${report.id}`}
              >
