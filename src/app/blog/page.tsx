...
export default function BlogPage() {
  ...
  const filteredPosts = useMemo(() => {
    let filtered = blogPosts.filter(post => !post.featured) // Excluir post em destaque
  ...
  const updatedCategories = useMemo(() => {
    return categories.map(cat => {
      if (cat.name === "Todos") {
        return { ...cat, count: blogPosts.length }
      }
      return {
        ...cat,
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {paginatedPosts.map((article) => (
                    <Card key={article.id} className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 group">
