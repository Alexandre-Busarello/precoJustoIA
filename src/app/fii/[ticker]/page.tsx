...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    if (!company) {
      return {
        title: `${ticker} - FII Não Encontrado | Análise Fácil`,
    ...
    const metadata = {
      ...
      openGraph: {
        ...
        description,
        type: 'article',
        url: `/fii/${tickerParam.toLowerCase()}`,
                    ...
                    {companyData.fiiData && (
                      <div className="mb-4 p-4 bg-blue-50 dark:bg-blue-950 rounded-lg border border-blue-200 dark:border-blue-800">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                          {companyData.fiiData.dividendYield && (
                            <div>
                              <span className="text-muted-foreground">Dividend Yield:</span>
                              <span className="ml-2 font-medium">
                                {(toNumber(companyData.fiiData.dividendYield)!).toFixed(2)}%
                              </span>
                            </div>
                          )}
                          {companyData.fiiData.valorPatrimonial && (
                            <div>
                              <span className="text-muted-foreground">Valor Patrimonial:</span>
                              <span className="ml-2 font-medium">
                                {formatCurrency(toNumber(companyData.fiiData.valorPatrimonial))}
                              </span>
                            </div>
                          )}
                          {companyData.fiiData.lastDividendValue && (
                            <div>
                              <span className="text-muted-foreground">Último Dividendo:</span>
                              <span className="ml-2 font-medium">
                                {formatCurrency(toNumber(companyData.fiiData.lastDividendValue))}
                              </span>
                            </div>
                          )}
                          {companyData.fiiData.patrimonioLiquido && (
                            <div>
                              <span className="text-muted-foreground">Patrimônio Líquido:</span>
                              <span className="ml-2 font-medium">
                                {formatCurrency(toNumber(companyData.fiiData.patrimonioLiquido))}
                              </span>
                            </div>
                          )}
                        </div>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "dividendYield": companyData.fiiData?.dividendYield ? toNumber(companyData.fiiData.dividendYield) : undefined,
              "netAssets": companyData.fiiData?.patrimonioLiquido ? toNumber(companyData.fiiData.patrimonioLiquido) : undefined,
