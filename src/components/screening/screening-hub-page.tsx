...
} from "@/components/fii-screening-configurator"
...
export function ScreeningHubPage({ variant }: { variant: ScreeningHubVariant }) {
  ...
  const assetType = searchParams.get('assetType') as 'b3' | 'bdr' | 'both' | 'fii' | null
  
  const assetTypeFilter = isFiisHub ? 'fii' : (assetType || 'both')
  ...
  useEffect(() => {
    setParams((p) => ({ ...p, assetTypeFilter: isFiisHub ? 'fii' : (assetType || 'both') }))
  }, [assetType, isFiisHub])
  ...
  const handleGenerateScreening = async (customParams?: ScreeningParams) => {
    ...
    try {
      ...
      const isFii = finalAssetTypeFilter === 'fii';
          ...
          isFii
            ...
            : {
                ...
                params: {
                  ...paramsWithoutLimit,
                  includeBDRs:
                    finalAssetTypeFilter === "both" ||
                    finalAssetTypeFilter === "bdr",
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-background dark:via-background dark:to-background">
        ...
        <div className="container mx-auto px-4 pt-6">
          <Breadcrumbs items={[
            ...
            { label: isFiisHub ? "Screening de FIIs" : "Screening de Ações" }
          ]} />
        </div>

        {/* Título simples */}
        <div className="container mx-auto px-4 pt-4 pb-8">
          <h1 className="text-3xl font-bold mb-2">{isFiisHub ? "Screening de FIIs B3" : "Screening de Ações B3"}</h1>
          <p className="text-muted-foreground">
            {isFiisHub
              ? "Filtre fundos imobiliários por DY, P/VP, liquidez, segmento e tipo (tijolo/papel)."
              : "Use filtros customizáveis para encontrar ações que atendem seus critérios exatos"}
          </p>
          {isFiisHub ? (
            <p className="text-sm text-muted-foreground mt-2">
              <Link href="/screening-acoes" className="text-blue-600 hover:underline">Screening de ações</Link>
              {" · "}
              <Link href="/ranking" className="text-blue-600 hover:underline">Rankings</Link>
            </p>
          ) : (
            <p className="text-sm text-muted-foreground mt-2">
              <Link href="/screening-fiis" className="text-blue-600 hover:underline">Screening de FIIs</Link>
              {" · "}
              <Link href="/ranking" className="text-blue-600 hover:underline">Rankings</Link>
            </p>
          )}
        ...
        {/* Atalhos Rápidos para Presets - apenas screening de ações */}
        {!isFiisHub && (
                ...
                <div className="flex justify-center pt-6">
                  ...
                  Processando filtros...
                      ...
                      <>
                        ...
                        {isFiisHub ? "Buscar FIIs" : "Buscar Empresas"}
                      </>
                    )}
                  </Button>
              </div>
            ...
            <div className="container mx-auto px-4 max-w-7xl">
              <div className="mb-6">
                <h2 className="text-2xl font-bold mb-2">
                  Resultados ({results.count} {isFiisHub ? "FIIs encontrados" : "empresas encontradas"})
                </h2>
                {results.rational && (
                  ...
                  <tbody>
                    {getPaginatedResults().map((result, index) => (
                      <tr key={index} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        <td className="p-3">
                          <Link href={results?.model === 'fiiScreening' ? `/fii/${result.ticker.toLowerCase()}` : `/acao/${result.ticker}`} className="flex items-center gap-2 hover:text-blue-600 transition-colors">
                        ...
                        >
                          {result.upside != null ? `${result.upside > 0 ? '+' : ''}${result.upside.toFixed(1)}%` : 'N/A'}
                        </td>
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start gap-3 mb-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <Link href={results?.model === 'fiiScreening' ? `/fii/${result.ticker.toLowerCase()}` : `/acao/${result.ticker}`} className="hover:text-blue-600 transition-colors">
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-background dark:via-background dark:to-background">
      ...
      <LandingHero
        ...
        subheadline={
          isFiisHub ? (
            <>
              Filtre <strong>fundos imobiliários</strong> por <strong>Dividend Yield, P/VP, liquidez, vacância, quantidade de imóveis</strong> e tipo{" "}
              <strong>tijolo ou papel</strong>. Mesma base de dados usada no ranking PJ-FII.
            </>
          ) : (
            <>
        ...
        socialProof={
          isFiisHub
            ? [
                { iconName: "Filter", text: "DY, P/VP, liquidez" },
                { iconName: "Building2", text: "FIIs B3" },
                { iconName: "Target", text: "Segmento e tipo" },
              ]
            : [
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-bold mb-4">
              Por que usar{" "}
              <span className="text-blue-600">{isFiisHub ? "Screening de FIIs?" : "Screening de Ações?"}</span>
            </h2>
            <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
              {isFiisHub
                ? "Compare dezenas de fundos imobiliários com filtros objetivos (DY, valuation, qualidade do portfólio) em um só lugar."
      ...
      <section id="screening-tool" className="py-16 sm:py-20 bg-gradient-to-b from-gray-50 to-white dark:from-background/50 dark:to-background">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          <div className="text-center mb-8">
            <h2 className="text-3xl sm:text-4xl font-bold mb-4">
              Experimente o{" "}
              <span className="text-blue-600">{isFiisHub ? "Screening de FIIs" : "Screening"}</span>
            </h2>
            <p className="text-lg text-muted-foreground">
              {isFiisHub
                ? "Configure filtros e encontre FIIs que combinam com sua tese de renda."
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  {getPaginatedResults().map((result, index) => (
                    <tr key={index} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      <td className="p-3">
                        <Link href={results?.model === 'fiiScreening' ? `/fii/${result.ticker.toLowerCase()}` : `/acao/${result.ticker}`} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                          <CompanyLogo ticker={result.ticker} logoUrl={result.logoUrl} size={32} companyName={result.name} />
                  <div>
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "mainEntity": [
                {
                  "@type": "Question",
                  "name": "Como funciona o screening de FIIs?",
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Você define filtros como Dividend Yield mínimo, P/VP máximo, liquidez mínima, vacância, segmento e tipo (tijolo ou papel). Retornamos apenas fundos imobiliários que passam em todos os critérios ativos."
                  }
                },
                {
                  "@type": "Question",
                  "name": "O screening de FIIs é gratuito?",
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Sim. A busca básica é gratuita com limite de resultados para visitantes; assinantes Premium veem a lista completa e métricas adicionais alinhadas ao score PJ-FII."
                  }
                },
                {
                  "@type": "Question",
                  "name": "Quais métricas de FII posso filtrar?",
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Dividend yield, P/VP, liquidez média diária, quantidade de imóveis, vacância média, segmento (ex.: logística, papel) e classificação tijolo vs papel."
                  }
                }
              ]
            })
